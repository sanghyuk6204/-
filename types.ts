...
export interface TrendRankItem {
    rank: number;
    keyword: string;
    change: number | 'new'; // numeric change or 'new'
    status: 'up' | 'down' | 'same' | 'new';
}

export interface TopicGroup {
    topic: string; // e.g., "Stocks", "Real Estate"
    keywords: TrendRankItem[];
}

export interface AgeGroupTrend {
    ageGroup: string; // e.g., "10s", "20s"
    keywords: TrendRankItem[];
}

export type SearchSource = 'google' | 'naver';

export type Feature = 'keywords' | 'blogs' | 'competition' | 'sustainable-topics' | 'related-keywords' | 'shopping' | 'trends' | 'category-trends';
